  ...
  if istty {
    loop {
      ...
      match std::io::stdin().read_line(&mut buffer) {
        Ok(_n) => {
          // emulates stdin JSON query from plain text
      ...
      loop {
        ...
        tokio::spawn(async move {
          loop {
            ...
            let (read, write) = socke
            socket.read_to_string(&mut buf).await.expect("failed to read from socket");
            let arc3 = arc2.clone();
            tokio::spawn(async move {
              let message = serde_json::from_str::<io::QueryMessage>(&*buf).expect("failed to deserialize query message");
              let query = Query::from_io(&message.query);
              let idx = &*arc3.read().unwrap();
              let result = search(&idx, &query);
              let out = io::Out { replyTo: message.id, result };
              socket.write_all(serde_json::to_string(&out).unwrap().as_bytes());
            });
          };
        });
      };
    ...
    */
    ...
    loop {
      ...
      let arc = index.clone();
      match std::io::stdin().read_line(&mut buffer) {
        Ok(_n) => match serde_json::from_str::<io::QueryMessage>(buffer.as_str()) {
          Ok(query_message) => {
            tokio::spawn(async move {
              let idx = &*arc.read().unwrap();
              ...
              println!("{}", serde_json::to_string(&io::Out {replyTo: query_message.id, result}).unwrap());
              /*
              log(format!(
                "[SEARCH] {}: {} results in {}ms",
                serde_json::to_string(&query_message).unwrap(),
                total,
                time
              ));
              */
          ...
          Err(e) => log(format!("[ERROR]: Cannot read json message: {}", e)),
        },
        Err(e) => log(format!("[ERROR]: Cannot read from stdin: {}", e)),
      }
    }
  }
}
