...
export const render = (entries: Entry[], baseUrl: string): string => {
  return `\
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xmlns:xhtml="http://www.w3.org/1999/xhtml"> 
  ${entries.map(entry =>
    ...
    ${entry.alternates.map(alt =>
      `<xhtml:link rel="alternate" href="${baseUrl + alt.href}" hreflang="${alt.lang}"/>`
    ).join("\n    ")}
  </url>`
  ).join("\n  ")}
</urlset>`;
...
const etaString = (ms: number): string => {
  const days = (ms / d) | 0;
...
const testSitemap = async (entries: Entry[], baseUrl: string) => {
  
  terminal.fullscreen(true);

  const agent = new Agent({keepAlive: true, keepAliveMsecs: 10000});
  ...
  */
  ...
  const errorLog = fs.createWriteStream(__dirname + "/errors.txt", "utf8");
  const addError = (error: [number, string]) => {
    errors.push(error);
  ...
  const render = () => {
    ...
    out.cursorTo(0, 0);
    wl("=".repeat(75));
    wl(chalk.blue(`Testing sitemap`));
    wl(chalk.green(baseUrl));
    wl(chalk.yellow(String(entries.length)) + " entries");
    wl(chalk.yellow(String(total)) + " urls (with alternates)");
    wl(`Concurrency: ${chalk.yellow(String(concurrency))}`);
    ...
    wl("=".repeat(75));
    wl("Elapsed: " + chalk.yellow((percent * 100).toFixed(2) + "%"));
    wl("ETA: " + etaString(elapsedMs / elapsed * remaining));
    wl(`Req/s: ${chalk.yellow(String(rs))} req/s`)
    wl(`AVG Response Time: ${chalk.yellow(String((responseTimeSum / elapsed) | 0))}ms`)
    ...
    for(const [code, count] of codes){
      wl(chalk.yellow(String(code || "Net Error")) + ": " + chalk.yellow(String(count)));
    }
    wl("=".repeat(75));
  ...
  const next = async () => {
    ...
    const start = now();
    try{
      const res = await fetch(baseUrl + url, {agent, redirect: "manual"});
...
const renderIndex = (pages: number, baseUrl: string) => {
  return `\
<?xml version="1.0" encoding="UTF-8"?>
...
const main = async () => {
  ...
  fs.writeFileSync(__dirname + "/sitemap-4.xml", renderIndex(numPages, baseUrl));

  try{ fs.mkdirSync(__dirname + "/sitemap-4") } catch(e){};

  for(let page = 0; page < numPages; page++){
    const start = page * pageSize;
    const end = start + pageSize;
    const slice = entries.slice(start, end);
    fs.writeFileSync(__dirname + `/sitemap-4/sitemap-${page + 1}.xml`, render(slice, baseUrl));
  }
